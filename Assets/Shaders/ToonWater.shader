Shader "ToonWater"
{
		...
		SubShader
	{
		...
		// amount consistent.
		float3 normalDot = saturate(dot(existingNormal, 0));

		// ## FIX FOR PS4/PLSL ##
		// NOTE: Seems to work just fine on dx as well... if not we'll need to add some platform directives.
		float foamDistance = lerp(fixed4(1, 1, 1, 1) * _FoamMaxDistance, fixed4(1, 1, 1, 1) * _FoamMinDistance, normalDot);
		// #################
